...
public class NetworkConnected2 {
    ...
    public static int makeConnected(int n, int[][] connections) {
        ...
        Set<Integer> checked = new HashSet();// 공유 자원

        // 그래프를 만드는 부분
        // 1. 배열 대신 인접 리스트 사용, 동적할당
        List<List<Integer>> graph = new ArrayList<>();// 공유 자원
        for (int i = 0; i < n; i++) {
...
//        System.out.println("최종 networkCount:" + networkCount);
        ...
        // 체킹이 되지 않은 개수
        int isolationNetwork = n - checked.size();

        return networkCount + isolationNetwork - 1;
    }

    private static void DFS(int startIndex, Set checked, List graph) {
//        System.out.println("call: " + startIndex);
        // 방문체크
        if (!checked.contains(startIndex) && checked.add(startIndex)) {
            // 연결된 곳으로 간다
            List nextList = (List) graph.get(startIndex);
            for (int i = 0; i < nextList.size(); i++) {
                int nextIndex = (int) nextList.get(i);
                if (!checked.contains(nextIndex)) {
//                    System.out.println("from: " + startIndex + ", to:" + i);
                    DFS(nextIndex, checked, graph);
                }
            }
